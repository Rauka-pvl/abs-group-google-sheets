...
if (nodeMajor < 18) {
    console.error(`Node.js ${process.versions.node} is too old. Install Node.js 18+ (recommended 20 LTS) in the hosting panel, then redeploy.`);
    process.exit(1);
}
...
app.post("/action", async (req, res) => {
    try {
        ...
        const body = req.body;
        console.log(`Incoming event=${body.event ?? "?"} id=${body.data?.FIELDS?.ID ?? "?"} entityTypeId=${body.data?.FIELDS?.ENTITY_TYPE_ID ?? "?"}`);
        ...
        console.log(`Processed #${result.bitrixId}: ${result.action}`);
        res.status(200).json({
            ok: true,
...
app.listen(config.port, () => {
    ...
    console.log(`Tracked SPA entityTypeId=${config.entityTypeId}, categories=${config.categoryIds.join(",")}`);
    console.log("Subscribe outgoing webhook to ONCRMDYNAMICITEMADD / ONCRMDYNAMICITEMUPDATE / ONCRMDYNAMICITEMDELETE");
});
