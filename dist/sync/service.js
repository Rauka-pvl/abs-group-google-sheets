...
import { fetchAccountingRowById, fetchAccountingRows, } from "../bitrix/accounting.js";
import { BitrixClient } from "../bitrix/client.js";
import { config } from "../config.js";
import { SheetsClient } from "../sheets/client.js";
import { markAccountingRowDeleted, syncAccountingToSheet, upsertAccountingRow, } from "../sheets/sync.js";
export function createBitrixClient() {
...
export async function createSheetsClient() {
    try {
        ...
        throw new Error(`Google service account file not found: ${config.googleServiceAccountPath}\n` +
            "Create a service account key, save it there, and share the spreadsheet with the SA email (Editor).");
    }
    return new SheetsClient(config.spreadsheetId, config.sheetName, config.googleServiceAccountPath);
}
...
export async function previewAccountingRows() {
    const bitrix = createBitrixClient();
    const rows = await fetchAccountingRows(bitrix);
    console.log(`Fetched ${rows.length} card(s) from Bitrix`);
    for (const row of rows) {
        console.log(`#${row.bitrixId} | ${row.categoryName} | ${row.title} | ${row.amount} | ${row.date} | ${row.status}`);
    }
}
...
export async function handleBitrixEvent(payload) {
    ...
    if (DEAL_EVENTS.has(event)) {
        return {
            status: "ignored",
            reason: "ONCRMDEAL* is for classic deals. Accounting funnels are SPA «Заявка в бухгалтерию» — use ONCRMDYNAMICITEMADD/UPDATE/DELETE",
        };
    }
