...
export const TECH_HEADERS = ["Bitrix ID", "Воронка"];
...
export class SheetsClient {
    ...
    sheets = null;
    layout;
    constructor(spreadsheetId, sheetName, serviceAccountPath, layout = {}) {
    ...
    async ensureHeader() {
        ...
        const businessOk = checks.every(([index, value]) => (header[index] ?? "").trim() === value);
        if (!businessOk) {
            throw new Error(`Sheet "${this.sheetName}" row ${headerRow} headers do not match. ` +
                `Expected B/D–I: ${BUSINESS_HEADERS.join(" | ")}`);
        }
        const needTechHeaders = (header[9] ?? "").trim() !== TECH_HEADERS[0] ||
